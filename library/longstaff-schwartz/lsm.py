...
def simulate_paths_gbm(
    S0, r, sigma, corr, T, M, N, seed=None, antithetic=False
):
    """
...
def _build_basis_with_cross(S: np.ndarray, degree: int = 2) -> np.ndarray:
    ...
    multi_indices = [
        idx for idx in product(range(degree + 1), repeat=d)
        if sum(idx) <= degree
    ]
...
def _build_basis_weighted_laguerre(S: np.ndarray, K: float, degree: int = 2) -> np.ndarray:
    """
